...
async function loadBookingHistory(uid) {
  ...
  try {
    ...
    historyBox.innerHTML = ''; // ล้างข้อความว่างเปล่าออกเตรียมใส่ข้อมูลจริง
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      // จัดการวันที่ (สมมติว่าเก็บเป็น String หรือดึงจาก createdAt)
      let dateStr = data.date || data.createdAt || '';
      
      // ถ้าระบบคุณเก็บวันที่เป็นแบบ Timestamp (Firebase Object) ให้ใช้แบบนี้แปลง
      if (data.createdAt && typeof data.createdAt.toDate === 'function') {
        const d = data.createdAt.toDate();
        dateStr = d.toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric' });
      }

      // นำข้อมูลมายัดใส่โครงสร้าง HTML ที่เราออกแบบไว้
      const html = `
...
/* ════════════════════════════════════════════
   ...
   (โค้ดส่วนนี้ของคุณสมบูรณ์อยู่แล้ว ผมเก็บไว้เหมือนเดิมครับ)
════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════
   PROFILE อัปเดตรูป, เซฟข้อมูล, ล็อกเอาท์
════════════════════════════════════════════ */
